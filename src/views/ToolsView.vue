<template>
    <a-button style="margin: 10px" @click="insert_visible = true">添加工具</a-button>
    <a-table
            :columns="columns"
            :data-source="data"
        ...
        <a-form>
            ...
            <a-form-item label="工具分类">
                <a-select v-model:value="edit_form.category" >
                    ...
                    <a-select-option value="news">AI绘画工具</a-select-option>
                    <a-select-option value="knowledge_map">AI视频工具</a-select-option>
                    <a-select-option value="hot_entry">热门工具</a-select-option>
                </a-select>
            </a-form-item>
            <a-form-item>
...
export default {
    ...
    setup() {
        ...
        function onEdit(record) {
            ...
            edit_form.value.href = record.href;
        ...
        async function onEditText(flag) {
            ...
            // }
            text.value = edit_form.value.href;
        }
        ...
        async function preview(record) {
            ...
            //     openNotification('error','获取词条信息失败', '请稍后再试')
            // }
            previewText.value = record.href;
        }

        async function fetchTools() {
            await api.get('/getTool').then((res) => {
                if (res.data.code === 20000) {
                    data.value = res.data.data.tools;
                } else {
                    openNotification('error','获取工具信息失败', '请稍后再试')
                }
            }).catch((err) => {
                console.log(err);
                openNotification('error','登录信息失效', '请重新登录')
